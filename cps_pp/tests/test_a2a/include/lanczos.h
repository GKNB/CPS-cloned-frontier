...
void testLanczosIO(typename GridA2Apolicies::FgridGFclass &lattice){
  ...
  GridLanczosDoubleConvSingle<GridA2Apolicies> lanc;
  lanc.randomizeEvecs(lanc_arg,lattice);

  auto lanc_iface = lanc.createInterface();
  Grid::GridBase* FGridF = lanc_iface->getEvecGridF();

  lanc.writeParallel("lanc");

  {
    GridLanczosDoubleConvSingle<GridA2Apolicies> lanc2;
    lanc2.readParallel("lanc");
    auto lanc2_iface = lanc2.createInterface();
    ...
    for(int i=0;i<lanc_iface->nEvecs();i++){
      double lanc_eval = lanc_iface->getEvecF(lanc_evec, i);
      double lanc2_eval = lanc2_iface->getEvecF(lanc2_evec, i);

      assert(lanc_eval == lanc2_eval);
      c_odd_d_1.importGridField(lanc_evec);
      c_odd_d_2.importGridField(lanc2_evec);
      
      assert( c_odd_d_1.equals( c_odd_d_2 ) );

      auto view = lanc_evec.View(Grid::CpuRead);
      auto view2 = lanc2_evec.View(Grid::CpuRead);

      for(int s=0;s<lanc_evec.Grid()->oSites();s++)
