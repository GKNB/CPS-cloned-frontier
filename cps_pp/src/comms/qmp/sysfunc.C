...
//Useful global variables
  ...
  void init_qmp ()
  {
    ERR.General ("", "init_qmp()",
                 "default arguments no loger supported. Call init_qmp(&argc,&argv) via (CPS_NAMESPACE)::Start(&argc,&argv)");
  }
...
//Get Allocated machine size, and declare logical machine
  ...
  void init_qmp (int *argc, char ***argv)
  {
    ...
    QMP_status_t init_status =
      QMP_init_msg_passing (argc, argv, QMP_THREAD_MULTIPLE, &prv);

    if (init_status)
      printf ("QMP_init_msg_passing returned %d\n", init_status);
...
#ifdef USE_GRID
    FgridBase::initializeGrid(argc,argv); //initialize Grid and generate Grids
...
int UniqueID ()
{
  return QMPSCU::peRank;
}

int CoorX ()
{
  if (!QMPSCU::initialized) {
    QMPSCU::init_qmp ();
  }
  return QMPSCU::pePos[0];
}
