...
CPS_START_NAMESPACE
/*!\file
	$Id: fix_gauge.C,v 1.9.32.6 2013/06/24 20:55:15 yujl Exp $
	*/
//--------------------------------------------------------------------
//  CVS keywords
//
//  $Author: yujl $
//  $Date: 2013/06/24 20:55:15 $
//  $Header: /space/cvs/cps/cps++/src/util/lattice/fix_gauge/fix_gauge.C,v 1.9.32.6 2013/06/24 20:55:15 yujl Exp $
//  $Id: fix_gauge.C,v 1.9.32.6 2013/06/24 20:55:15 yujl Exp $
//  $Name:  $
//  $Locker:  $
//  $Revision: 1.9.32.6 $
...
class HyperPlane
{
	...
	protected:
		...
		const Matrix& L_loc(int ind_link, int * index) const;
		const Matrix& L(int ind_link, int * index) const;
	...
	protected:
		...
		int  hplane_vol;    // volume of the hyperplane
	...
	private:
		...
		void copy_nbr_links(int nbr_num, int recurse, int * index);
};
	...
	commu_time(0.0)
{
	...
	//-----------------------------------------------------------------------
	{
		...
		for(int i=0; i<hplane_dim; i++)
		{
			...
			int index[hplane_dim];
			copy_nbr_links(i, 0, index);
		}
	}
	//-------------------------------------------------------------------------------
...
void HyperPlane::copy_nbr_links(int nbr_num, int recurse, int * index)
{
	...
	if(recurse < hplane_dim) // index has not been completed
		if(recurse == nbr_num)
		{
			copy_nbr_links(nbr_num, recurse + 1, index); // skip this field in index
		}
		else
		{
			...
			{
				index[recurse] = i;
				copy_nbr_links(nbr_num, recurse + 1, index);
			}
		}
	else  // index is completed
	{
		...
		index[nbr_num] = - 1;
		void * rec = (void*) &L(nbr_num, index);

		index[nbr_num] = node_size[nbr_num] - 1;
		getMinusData((IFloat*)rec, (IFloat*)&L_loc(nbr_num, index),
...
#pragma omp parallel for
		for(int site = 0; site < hplane_vol; site++)
		{
			int index[hplane_dim];
			int idx_sum = cal_idx(site, index);
			if(idx_sum % 2 == 0) fix_g(G_loc(index), index);
		}
		copy_nbr_gauge();
		//Odd part
#pragma omp parallel for
		for(int site = 0; site < hplane_vol; site++)
		{
			int index[hplane_dim];
			int idx_sum = cal_idx(site, index);
			if(idx_sum % 2 == 1) fix_g(G_loc(index), index);
		}
		copy_nbr_gauge();
	}
...
//}
//
//void FixHPlane::iter(int recurse, int dist, int * index)
//{
//	char *fname = "iter(i,i)";
//
//	VRB.Func(cname, fname);
...
//		index[recurse] = xx;
//		if(recurse > 0)
//			iter(recurse-1, dist-xx, index);
//		else
//			fix_g(G_loc(index),index);
//	}
//}
...
void FixHPlane::findA(Matrix & Abuf, int * index)
{
	...
	for(int i=0; i<hplane_dim; i++)
	{
		...
		if ( ind2dir[i] == GJP.XiDir()) {
			tmp_m1 = L(i, index);
			tmp_m1 *= GJP.XiGfix();
			tmp_m.DotMPlus(G(i, index), tmp_m1);
		} else {
			tmp_m.DotMPlus(G(i, index), L(i, index));
		}
		// End modification

		index[i] += 2;
		Matrix& G_plus_i = G(i, index);

		index[i]--;
		tmp_m1.Dagger(L_loc(i, index)); // local link
	...
	omp_set_num_threads(64);
	{
		...
		for(int i=0; i < loop_num; i++)
		{
			...
			if( fix_gauge_ptr[i] != NULL )
			{
				...
				while((hplane.delta(dlt) > hplane.small_enough)
						&& (iternum < MaxIterNum))
				{
					//----------------------------------------------------------
					// Make iterations
					...
					VRB.Result(cname, fname, "Iter = %d, Delta = %e \n", iternum, dlt);
