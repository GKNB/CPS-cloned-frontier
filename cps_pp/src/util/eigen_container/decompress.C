...
int EvecReader::globalToLocalCanonicalBlock(int slot,const std::vector<int>& src_nodes,int nb) {
...
//	Lanczos_5d<lanc_type>* eig = (Lanczos_5d<lanc_type>*)(lat.get_eig());
	...
	vol4d = GJP.NodeSites(0) * GJP.NodeSites(1) * GJP.NodeSites(2) * GJP.NodeSites(3);
...
//		eig->bl[k] = vals[k];
	...
	FILE* f = NULL;
	uint32_t status = 0;
	if (UniqueID() == 0) std::cout << "here" << std::endl;
	if (UniqueID() == 0) {
...
#undef _IRL_READ_INT
	...
	if (UniqueID() == 0)
		printf("Reading data that was generated on node-layout: %d %d %d %d %d\n ",nn[0],nn[1],
				nn[2],nn[3],nn[4]);
	...
	for (int i=0;i<nb_per_node;i++) {
		block_data_ortho[i].resize(f_size_block * args.nkeep);    
		memset(&block_data_ortho[i][0], 0, f_size_block * args.nkeep* sizeof(float));
	}
	...
	for (std::map<int, std::vector<int> >:: iterator sl=slots.begin();sl!=slots.end();sl++) {
...
if(cdir){
		...
		sprintf(buf,"%s/%2.2d/%10.10d.compressed",path.c_str(),slot/nperdir,slot);
		f = fopen(buf,"rb");
		if (!f) {
...
#pragma omp for
							for (int i=0;i<nsingleCap;i++) {
								...
								memcpy(&block_data_ortho[mnb][i*buff.size()], &buff[0], buff.size() * sizeof(float)); 
							}
						}
					} 
				}
...
//#pragma omp parallel
				...
				for (int nb=0;nb<args.blocks;nb++) {
					int mnb = globalToLocalCanonicalBlock(slot,_nn,nb);
					if (mnb != -1){
						int64_t read_size = FP_16_SIZE( 2*_cf_block_size, 24 )*(args.nkeep - nsingleCap);
						int64_t seek_size = _cf_block_size * 2*nsingleCap*args.blocks*4 + 
							(args.nkeep - nsingleCap)*nb * FP_16_SIZE(2*_cf_block_size, 24 );
...
#pragma omp for
			...
			if (UniqueID() == 0) std::cout << "Finished reading block_data_ortho nsingleCap-nkeep " << std::endl;
			sync();

			int64_t seek_size = _cf_block_size * 2*nsingleCap*args.blocks*4 + 
				FP_16_SIZE( _cf_block_size*2*(args.nkeep - nsingleCap)*args.blocks, 24 );
			fseeko(f, seek_size, SEEK_SET);
			int64_t read_size = (4*args.nkeep_single*2 + FP_16_SIZE((args.nkeep - args.nkeep_single)*2, args.FP16_COEF_EXP_SHARE_FLOATS))
			...
			if (crc_comp != slot_checksums[2*args.blocks]) {
				printf("Reading block_coef crc_compute = %X, crc_read[2*blocks] = %X\n", crc_comp, slot_checksums[2*args.blocks]);
...
#pragma omp for
					...
					for (int nb=0;nb<args.blocks;nb++) {
					  ...
					  //int mnb = (nb < nb_per_node) ? nb : -1;
						if (mnb != -1) {
							...
							char* lptr = ptr + (4*buf1.size() + FP_16_SIZE(buf2.size(), args.FP16_COEF_EXP_SHARE_FLOATS))*
								(nb + j*args.blocks);
							int l;
							read_floats(lptr, &buf1[0], buf1.size() );
							//automatically increase lptr
							memcpy(&block_coef[mnb][j* (buf1.size() + buf2.size())], &buf1[0], buf1.size() * sizeof(float));
							//for (l=0;l<nkeep_single;l++) {
							//	((CoeffCoarse_t*)&coef._v[j]._odata[oi]._internal._internal[l])[ii] = CoeffCoarse_t(buf1[2*l+0],buf1[2*l+1]);
							//}
							read_floats_fp16(lptr, &buf2[0], buf2.size(), args.FP16_COEF_EXP_SHARE_FLOATS);
							memcpy(&block_coef[mnb][j* (buf1.size() + buf2.size()) + buf1.size()], &buf2[0], buf2.size() * sizeof(float));
			...
			if (UniqueID() == 0) std::cout << "Processed compressed data in " << t1 << " sec " << std::endl;
		//}
	}
	sync();
			...
			for (int nb=0;nb<nb_per_node;nb++) {
...

				{
					...
					for (int i=0;i<args.nkeep;i++) {
						...
						caxpy_single(dest_block, *(complex<float>*)coef, ev_i, dest_block, f_size_block);
					}
				}
			}
