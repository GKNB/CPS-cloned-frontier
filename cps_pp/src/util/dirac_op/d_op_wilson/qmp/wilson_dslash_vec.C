		       ...
		       unsigned long vec_offset)
{
  ...
  if(gparity_init_status == 1 && !GJP.Gparity() ||  gparity_init_status == 0 && GJP.Gparity()) init_len = -1;
  ...
  if(vec_len!=init_len){ 
    ...
    for (int i=0;i<4;i++){
      if ( GJP.Nodes(i) > 1 ){
	local_comm[i] = 0;
	if(!UniqueID()){ printf("Non-local in direction %d, sites on wall %d\n",i, num_nl[i]); fflush(stdout); }
      }
	...
	for(z=0; z<lz; z++){
	  for(t=0; t<lt; t++){
	    ...
	    if(parity == cbn){ 
	      ...
	      if(GJP.Gparity()) chi_f1 = chi_p + SPINOR_SIZE * (xyzt + vol);
		...
		if( (pos[mu] == lattsz[mu]-1) && !local_comm[mu] ){ //posp[mu]==0
		  ...
		  if(!GJP.Gparity()){
		    ...
		    psi = psi_p + SPINOR_SIZE * posp_xyzt;
		    MOVE_VEC(ind_buf[dir]+psi_f0_bufoff,psi, vec_len,vec_offset);

		    psi_f1 = psi_p + SPINOR_SIZE * (posp_xyzt + vol);
		    MOVE_VEC(ind_buf[dir]+psi_f1_bufoff,psi_f1, vec_len,vec_offset);
		  }
		...
		if(GJP.Gparity()){
		  ...
		  psi_f1 = psi_p + SPINOR_SIZE * (posm_xyzt + vol);
			...
			psi_f1_bufoff = 0;
		      ...
		      moveMem((IFloat *)(ind_buf[dir]+psi_f0_bufoff), (IFloat *)temps[mu][1], 
			      SPINOR_SIZE*sizeof(Float) / sizeof(char));
		      psi = psi + vec_offset;

		      moveMem((IFloat *)(ind_buf[dir]+psi_f1_bufoff), (IFloat *)(temps[mu][1]+SPINOR_SIZE), 
			      SPINOR_SIZE*sizeof(Float) / sizeof(char));
  ...
  for(index = 0; index<vol*2;index++){
	...
	if(GJP.Gparity()){
	  ...
	  //not sure if this actually *does* anything!
      	  psi = fbuf;
	  if(GJP.Gparity()) psi_f1 = fbuf+SPINOR_SIZE;
      	}else{
	  //internal site or local comms for edge sites
		...
		psi_f1_use = psi;
	      ...
	      MINUSMU(mu, u_f1, tmp, temps[mu][0]+SPINOR_SIZE,sdag,psi_f1_use);
		...
		for(c=0;c<3;c++){
		  for(r=0;r<2;r++){
		    if(mu==0){
		      FERM(chi,r,c,s) = FERM(temps[mu][0],r,c,s);
		      FERM(chi_f1,r,c,s) = FERM(temps[mu][0]+SPINOR_SIZE,r,c,s);
		    }else{
		      FERM(chi,r,c,s) += FERM(temps[mu][0],r,c,s);
		      FERM(chi_f1,r,c,s) += FERM(temps[mu][0]+SPINOR_SIZE,r,c,s);
		    }
	...
	if(GJP.Gparity()){
	  ...
	  if(GJP.Gparity()){
	    moveMem((IFloat *)(temps[mu][1]+SPINOR_SIZE), (IFloat *)(fbuf+SPINOR_SIZE), 
		    SPINOR_SIZE*sizeof(Float) / sizeof(char));
	  }
      	} else{
      	  for(int vec_ind=0;vec_ind<vec_len;vec_ind++){
		...
		for(c=0;c<3;c++){
		  for(r=0;r<2;r++){
		    ...
		    FERM(chi_f1_use,r,c,s) += FERM(temps[mu][1]+SPINOR_SIZE,r,c,s);
		  }
		}
	      }
