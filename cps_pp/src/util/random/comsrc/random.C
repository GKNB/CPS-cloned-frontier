...
//			  + OFFSET * (x[0]/2 + vx[0]*
		...
		start_seed_stacked_4d = start_seed_4d + n_rgen_4d_glb/2*OFFSET;
...
#ifdef RNG_WARMUP
{
		...
		index++;stk_index++;
		  ...
		  VRB.RNGSeed(cname,fname, "%d %d %d %d %d index=%d start_seed= %d\n",
			x[0],x[1],x[2],x[3],x[4],index,start_seed);
...
//		int new_seed = ugran[index].Urand(RandomGenerator::MBIG,0);
		int new_seed = rng_seed_5d.Urand(RandomGenerator::MBIG,0);
			rng_count++;
		VRB.RNGSeed(cname,fname,"index=%d start_seed=%d new_seed=%d\n",index,start_seed,new_seed);
...
//		int new_seed = ugran[index_4d].Urand(RandomGenerator::MBIG,0);
		int new_seed = rng_seed_4d.Urand(RandomGenerator::MBIG,0);
			rng_count_4d++;
		VRB.RNGSeed(cname,fname,"index_4d=%d start_seed_4d=%d new_seed=%d\n",index_4d,start_seed_4d,new_seed);
