...
CPS_START_NAMESPACE typedef long int SeedInt;
...
//  int index, index_4d;
  ...
  SeedList.iterator SeedIterator;
	...
	for (x[0] = x_o[0]; x[0] <= x_f[0]; x[0] += 2) {
	  ...
	  if (GJP.StartSeedKind () == START_SEED ||
	    ...
	    while (rng_count < rng_offset) {
	      SeedInt temp = ran_std ();
	      SeedIterator i = find (SeedList.begin (), SeedList.end (), temp);
	      if (i != SeedList.end ())
		ERR.General (cname, fname,
...
*/
int LatRanGen::StateSize () const const
{
...
*/
int LatRanGen::NStates (FermionFieldDimension frm_dim) const const
{
  return n_rgen_4d;
}
...
*/
void LatRanGen::GetState (RNGSTATE * state, FermionFieldDimension frm_dim) const const
{
...
/*!
  ...
  then the special RNG four gauge fields with domain-wall fermions is used.  
*/
void LatRanGen::GetStates (unsigned int **s, FermionFieldDimension frm_dim) const const
{
