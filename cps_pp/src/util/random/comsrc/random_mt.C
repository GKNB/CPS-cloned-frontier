...
//  cpsran_dump.open("cpsran_dump");
  ...
  int ngp = GJP.Gparity() + 1;

  n_rgen_4d = ngp * GJP.VolNodeSites () / 16;
...
// if(!cpsran) ERR.Pointer(cname, fname, "cpsran"); 
  ...
  if (GJP.StartSeedKind () == START_SEED_FIXED_UNIFORM) {
    ...
    for(int f=0; f<ngp; f++){ //Gparity flavor
...
LatRanGen::LatRanGen (const LatRanGen & in):
  ...
  cpsran(in.cpsran), urand(in.urand), grand(in.grand), 
  urand_lo(in.urand_lo), urand_hi(in.urand_hi), grand_mean(in.grand_mean), grand_sigma(in.grand_sigma)
{
...
LatRanGen & LatRanGen::operator= (const LatRanGen & in)
{
  ...
  urand = in.urand;
  grand = in.grand;
  urand_lo = in.urand_lo;
  urand_hi = in.urand_hi;
...
{
  if (is_initialized != in.is_initialized || n_rgen_4d != in.n_rgen_4d
      || rgen_pos_4d != in.rgen_pos_4d) {
    printf ("False on integers %d:%d  %d:%d  %d:%d\n", 
	    is_initialized, in.is_initialized,
	    n_rgen_4d, in.n_rgen_4d, 
	    rgen_pos_4d, in.rgen_pos_4d);
  ...
  std::vector<RNGSTATE> mystate(n_rgen_4d * this->StateSize());
  const_cast<LatRanGen*>(this)->GetAllStates(mystate.data());

  std::vector<RNGSTATE> instate(n_rgen_4d * this->StateSize());
  const_cast<LatRanGen&>(in).GetAllStates(instate.data());

  for (int i = 0; i < n_rgen_4d * this->StateSize(); i++) {
    if (mystate[i] != instate[i]) {
      std::cout << "False on RNG state " << i << " " << mystate[i] << ":" << instate[i] << std::endl;
      return false;
    }
  }
  return true;
...
{
  AssignGenerator (x[0], x[1], x[2], x[3], field_idx);
}
