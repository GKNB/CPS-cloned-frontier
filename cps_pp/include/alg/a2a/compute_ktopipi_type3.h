...
void ComputeKtoPiPiGparity<mf_Float>::type3_contract(KtoPiPiGparityResultsContainer &result, const int t_K, const int t_dis, const int thread_id, 
  ...
  for(int mu=0;mu<4;mu++){ //sum over mu here
	...
	SpinColorFlavorMatrix G1_pt1 = part1[pt1_pion]; //= G1*part1[pt1_pion];
	multGammaLeft(G1_pt1,1,gcombidx,mu);

	Matrix tr_sf_G1_pt1 = G1_pt1.SpinFlavorTrace();

	SpinColorFlavorMatrix G2_pt2_L = part2_L; //= G2*part2_L;
	multGammaLeft(G2_pt2_L,2,gcombidx,mu);

	Matrix tr_sf_G2_pt2_L = G2_pt2_L.SpinFlavorTrace();

	SpinColorFlavorMatrix G2_pt2_H = part2_H; // = G2*part2_H;
	multGammaLeft(G2_pt2_H,2,gcombidx,mu);

	Matrix tr_sf_G2_pt2_H = G2_pt2_H.SpinFlavorTrace();

	SpinColorFlavorMatrix ctrans_G2_pt2_L(G2_pt2_L); //speedup by transposing part 1
	ctrans_G2_pt2_L.transpose_color();
		
	FlavorSpinMatrix tr_c_G1_pt1 = G1_pt1.ColorTrace();
...
template<typename mf_Float>
void ComputeKtoPiPiGparity<mf_Float>::type3_compute_mfproducts(std::vector<std::vector<A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> > > &con_pi1_pi2_k,
  ...
  A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorVfftw> tmp_WV;
  A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> tmp_pi1pi2, tmp_pi2pi1;
  ...
  for(int pidx=0;pidx<nmom;pidx++){ //Average over momentum orientations
	...
	A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> *into_pi1_pi2 = pidx == 0 ? &con_pi1_pi2_k[tpi1_idx][tkp] : &tmp_pi1pi2;
	A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> *into_pi2_pi1 = pidx == 0 ? &con_pi2_pi1_k[tpi1_idx][tkp] : &tmp_pi2pi1;
...
template<typename mf_Float>
void ComputeKtoPiPiGparity<mf_Float>::type3_precompute_part1(std::vector<SpinColorFlavorMatrix> &mult_vMv_contracted_part1_pi1_pi2,
...
void ComputeKtoPiPiGparity<mf_Float>::type3(KtoPiPiGparityResultsContainer result[], KtoPiPiGparityMixDiagResultsContainer mix3[],
  ...
  //Form the product of the three meson fields
  //con_*_*_k = [[ wL^dag(y) S_2 vL(y) ]] [[ wL^dag(z) S_2 vL(z) ]] [[ wL^dag(x_K) wH(x_K) ]]
  std::vector<std::vector<A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> > > con_pi1_pi2_k; // [tpi1_idx][tkp]
  std::vector<std::vector<A2AmesonField<mf_Float,A2AvectorWfftw,A2AvectorWfftw> > > con_pi2_pi1_k;  
  ...
  for(int top_loc = 0; top_loc < GJP.TnodeSites(); top_loc++){
    ...
    std::vector<SpinColorFlavorMatrix> part2_L(size_3d); //[x3d]
    std::vector<SpinColorFlavorMatrix> part2_H(size_3d); //[x3d]
...
# ifndef DISABLE_TYPE3_PRECOMPUTE
	//Contract on all 3d sites on this node with fixed operator time coord top_glb into a canonically ordered output vector
	std::vector<SpinColorFlavorMatrix> mult_vMv_contracted_part1_pi1_pi2;  //[x3d];
	std::vector<SpinColorFlavorMatrix> mult_vMv_contracted_part1_pi2_pi1;  //[x3d];
