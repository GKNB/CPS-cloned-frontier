...
//      int vol_5d = vol_4d_oo * ls;
      ...
      int regu_coor =
        (pos[0] +
         s_l[0] * (pos[1] +
                   s_l[1] * (pos[2] + s_l[2] * (pos[3] % NtHalf)))) / 2;
...
//      crc = crc32_fast (buf, s, crc);
      ...
      if ((ret = fwrite (buf, s, 1, f)) != 1) {
        ERR.General(cname,fname, "Write failed! %d\n", ret);
      }
      double t1 = dclock ();
...
//          printf ("half first second = %d %x %x\n", half, first, second);
            std::cout << "half " << half << " first " << first << " second " <<
              second << std::endl;
          }
...
//      machine_is_little_endian = machine_endian ();
    ...
    int writeCompressedVector (const char *dir, OPT * V,
                               struct evec_write &warg,
...
// can assume that v >=0 and need to guarantee that unmap_fp16_exp(map_fp16_exp(v)) >= v
    ...
    void write_floats_fp16 (FILE * f, uint32_t & crc, OPT * in, int64_t n,
                            int nsc)
    {
      ...
      int64_t nsites = n / nsc;
      if (n % nsc) {
        ERR.General(cname,fname, "Invalid size in write_floats_fp16 %ld %d\n", n, nsc);
      }

      unsigned short *buf =
        (unsigned short *) malloc (sizeof (short) * (n + nsites));
      if (!buf) {
        ERR.General(cname,fname, "Out of mem\n");
      }
      // do for each site
