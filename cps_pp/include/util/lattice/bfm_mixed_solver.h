...
namespace mixed_cg {
  ...
  */
  ...
  //
  // max_cycle: the maximum number of restarts will be performed.
  inline int threaded_cg_mixed_multi_shift_MdagM(Fermion_t sol[], 
					  Fermion_t src,
					  double    mass[],
					  double    alpha[],
					  int       nshift,
    ...
    if(bfm_f.isBoss() && !me) printf("threaded_cg_mixed_multi_shift_MdagM: Doing initial single precision solve\n");
    {
    ...
    if(bfm_f.isBoss() && !me) printf("threaded_cg_mixed_multi_shift_MdagM: Starting main iteration loop\n");
	...
	if(!finished[n]){
	  ...
	  if(bfm_f.isBoss() && !me) printf("threaded_cg_mixed_multi_shift_MdagM: iter = %d shift = %d rsd = %17.10e(d) stop = %17.10e(d) finished = %d\n", i,n, norm, stop[n],finished[n]);
    ...
    if(bfm_f.isBoss() && !me) printf("threaded_cg_mixed_multi_shift_MdagM: Running double precision multi-mass using single precision version as guess");
