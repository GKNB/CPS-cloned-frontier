...
{
  public:
    static constexpr char* cname = "CPSQuda";
    ...
    static QudaPrecision setPrecision(CudaPrecision prec)
    {
      ...
      switch(prec)
      {
        case CUDA_HALF_PRECISION:
          VRB.Result("", fname, "CUDA_HALF_PRECISION\n");
          return QUDA_HALF_PRECISION;
        case CUDA_SINGLE_PRECISION:
          VRB.Result("", fname, "CUDA_SINGLE_PRECISION\n");
          return QUDA_SINGLE_PRECISION;
        case CUDA_DOUBLE_PRECISION:
          VRB.Result("", fname, "CUDA_DOUBLE_PRECISION\n");
          return QUDA_DOUBLE_PRECISION;
        default:
          ERR.General("", fname, "Undefined precision %d\n", prec);
      }
    }
    ...
    static QudaReconstructType setReconstruct(CudaReconstructType recon)
    {
      ...
      switch(recon)
      {
        ...
        default:
          ERR.General("", fname, "Undefined reconstruct type %d\n", recon);
      }
    }

    static void ParamSetup_dwf(QudaArg& quda_param, QudaGaugeParam& gauge_param, 
        QudaInvertParam& inv_param, int mat_type = 1)
    {
      const char* fname = "ParamSetup_dwf(QudaArg&, QudaGaugeParam&, QudaInvertParam&, i)";
